...
pub const DEFAULT_OKX_RPC_URL: &str = "https://exchainrpc.okx.com";
...
mod tests {
    ...
    fn wallet_network_info_maps_public_network_names() {
        ...
        assert_eq!(wallet_network_info(networks::OKX).unwrap().id, networks::OKX);
        assert_eq!(wallet_network_info(networks::BITCOIN).unwrap().id, networks::BITCOIN);
        assert_eq!(
