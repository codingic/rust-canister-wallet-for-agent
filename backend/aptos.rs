...
const CHAIN_NAME: &str = "aptos-mainnet";
...
pub fn get_balance(req: BalanceRequest) -> WalletResult<BalanceResponse> {
    ...
    Ok(BalanceResponse {
        ...
        block_ref: None,
        pending: true,
        message: Some("Aptos balance query not implemented yet".to_string()),
    })
}
...
pub fn transfer(req: TransferRequest) -> WalletResult<TransferResponse> {
    ...
    Ok(TransferResponse {
        ...
        tx_id: None,
        message: format!(
            "{CHAIN_NAME} transfer scaffold received request; signing/execution not implemented"
        ),
    })
}
